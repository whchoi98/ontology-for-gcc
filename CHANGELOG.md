...
### Fixed
- **Object Explorer 관계도 복구** (ADR-0022): offer/coupon 노드가 잘못된 합성 pk(=campaign_cd)로 MERGE되어 엣지가 orphan에 붙던 문제 — NODE_MAP pk를 자연키(offer_cd/coupon_no)로 정렬, `_TYPE_REGISTRY` id_prop 동기화, PRICED_AT를 합성 price_id 매칭으로, 엣지 적재기를 MERGE→MATCH(orphan 미생성, Plan-5 write-back 패턴)로 변경. 정렬 불변식 테스트 8종 + 검증 스크립트 `scripts/probe_object_edges.py` + 재적재 Runbook 06 추가. (라이브 반영은 Neptune 엣지 재적재 필요.)
- **Object Explorer empty relationship graph** (ADR-0022): aligned node MERGE pk with edge match-fields (offer_cd/coupon_no/price_id), switched edge endpoints from MERGE to MATCH to stop orphan-stub creation. Requires a Neptune edge reload (Runbook 06) to take effect on live data.
