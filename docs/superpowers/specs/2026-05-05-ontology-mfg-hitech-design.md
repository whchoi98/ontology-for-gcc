...
| D9 | Cognito 시드 | `admin@whchoi.net` (admin 그룹) / `demo@whchoi.net` (페르소나 5명 시연용). PW `!234Qwer` 영구 고정 |
...
| 시드 사용자 | – | `admin@whchoi.net` (admin), `demo@whchoi.net` (페르소나 5명 시연용). PW `!234Qwer` 영구 |
...
| 시드 사용자 | `admin@whchoi.net` (admin 그룹) / `demo@whchoi.net` (buyer 기본 그룹, 시연 시 PersonaSwitch로 5 페르소나 모두 접근). PW `!234Qwer` 영구 |
...
| 시드 비밀번호 `!234Qwer` 영업 노출 | 낮 | 데모 계정 탈취 | 데모 환경 전용, prod 격상 시 폐기 + Secrets Manager로 회전, 시연 후 IP 기반 access 제한 옵션 |
...
| 시드 비밀번호 회전 | 데모 환경 영구 `!234Qwer` | 운영 격상 시 Secrets Manager 회전 |
